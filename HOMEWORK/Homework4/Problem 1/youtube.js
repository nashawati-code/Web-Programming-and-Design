var youtube = {
    /* Expects an argument that is either a youtube URL or a ID, and returns back the ID. */
    ...
    /* Expects an argument that is either a youtube URL or a ID, and returns back the URL of the thumbnail for that video. */
    generateThumbnailUrl: function(videoIdOrUrl) {
        return 'https://i3.ytimg.com/vi/' + youtube.getIdFromUrl(videoIdOrUrl) + '/default.jpg';
    },
    /* Expects an argument that is either a youtube URL or a ID, and returns back the URL for that video. */
